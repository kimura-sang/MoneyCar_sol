...
contract MoneyCar {
    ...
    function withdraw() external nonReentrant{
        ...
        withdrawToId(userId[msg.sender], amount);
    ...
    function _upgrade(uint256 _userId) internal {
        uint8 userCycle = users[_userId].cycle;
        uint256 userReinvests = users[_userId].reinvests;
    ...
    function upgradeAdmin(uint256 _userId) external nonReentrant  {
        require(msg.sender == _creator, "You're not authorized to call this function");
        _upgrade(_userId);
    }

    function upgradeMe() external nonReentrant {
        require(userId[msg.sender] != 0, "You're not authorized to call this function");
        _upgrade(userId[msg.sender]);
    }
